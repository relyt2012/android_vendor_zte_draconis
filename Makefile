...
LINEEND=" \\"
COUNT=`wc -l ../draconis/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../draconis/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../draconis/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
